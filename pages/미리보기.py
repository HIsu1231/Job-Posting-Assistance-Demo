...
with col2:
    if st.session_state['publication'] == "직접 설정":
        publication_date = st.session_state["date_of_publication"]
    else:
    ...
    if st.session_state['Deadline'] == "직접 설정":
        deadline_date = st.session_state['deadline_date']
    else:
        deadline_date = st.session_state['Deadline']

    # 마감일을 텍스트로 변환하여 st.write로 출력
    deadline_date_str = deadline_date.strftime("%Y-%m-%d") if isinstance(deadline_date, datetime) else str(
        deadline_date)
    st.write("공고 마감일: ", deadline_date_str)
